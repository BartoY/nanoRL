...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, past_key_value=None, use_cache=False):
        ...
        if self.flash:
            # 如果增量解码 (T=1) 且有 past_key_value，其实已经隐式保证了因果性，is_causal 可以为 False
            ...
            if T > 1 and past_key_value is None:
...
class Block(nn.Module):
    ...
    def forward(self, x, past_key_value=None, use_cache=False):
        attn_out, new_kv = self.attn(self.ln_1(x), past_key_value=past_key_value, use_cache=use_cache)
        x = x + attn_out
...
class GPTDecoder(nn.Module):
    ...
    def forward(self, encoder_out, mach_dense, mask_padding, mask_machine_compat, op_proc_time, job_length,
                rollout=False, temperature=1.0):
        ...
        # 初始化 KV Cache
        # 用于缓存每一层 Block 的 (key, value) 元组
        past_key_values = [None] * len(self.h)

        for i in range(max_steps):
            # 1. 增量解码：只处理当前最新的一步
            x = curr_input.unsqueeze(1)  # [B, 1, H]

            with torch.no_grad():
                pos = torch.full((bsz, 1), i, dtype=torch.long, device=device)  # [B, 1]
            pos_emb = self.wpe(pos)  # [B, 1, H]

            x = self.drop(x + pos_emb)

            # 逐层应用 KV Cache 进行 Forward
            new_past_key_values = []
            for block_idx, block in enumerate(self.h):
                past_kv = past_key_values[block_idx]
                x, new_kv = block(x, past_key_value=past_kv, use_cache=True)
                new_past_key_values.append(new_kv)

            past_key_values = new_past_key_values  # 更新 Cache
            x = self.ln_f(x)  # [B, 1, H]
            hx = x.squeeze(1)  # [B, H] (拿到当前步的特征)

            # High-Level 指针注意力
            q_op = hx.unsqueeze(1)
            glimpse_op = self.op_pointer_att(q=q_op, k=encoder_out, v=encoder_out, mask=mask_padding)

            # 2. FJSP 内部状态逻辑转移使用 torch.no_grad 隔离梯度，节省极大显存
            with torch.no_grad():
                current_global_indices = base_indices + job_next_op_local_idx
                safe_indices = current_global_indices.clamp(max=n_node - 1)

            # k_candidates 依旧需要梯度，因为它来自于 encoder_out
            ...
            with torch.no_grad():
                ...
                min_comp_mach_avail = valid_avail_time.min(dim=-1)[0]
                min_comp_mach_avail = min_comp_mach_avail.masked_fill(min_comp_mach_avail == float('inf'), 0.0)

                e_start = torch.max(job_ready_time, min_comp_mach_avail)
                e_comp = e_start + cand_pt_est
                wait_time = e_start - job_ready_time
                idle_time = e_start - min_comp_mach_avail
                ops_left_ratio = (job_length - job_next_op_local_idx) / job_length.clamp(min=1)
                ops_left_ratio = ops_left_ratio.float()
                pt_ratio = cand_pt_est / (cand_pt_est.mean(dim=-1, keepdim=True) + 1e-5)
                ...
                progress_diff = job_progress - mean_progress
            ...
            selected_op_emb = torch.gather(encoder_out, 1, idx_exp.expand(-1, -1, self.hidden_dim)).squeeze(1)
            ...
            with torch.no_grad():
                curr_op_mach_compat = torch.gather(mask_machine_compat, 1, idx_exp.expand(-1, -1, n_m)).squeeze(1)
                ...
                chosen_job_ready = torch.gather(job_ready_time, 1, sel_job_unsq)
                est_start = torch.max(chosen_job_ready, machine_avail_time)
                est_comp = est_start + curr_op_pt
                ...
                norm_mach_avail = (machine_avail_time / max_avail).unsqueeze(-1)
                norm_curr_op_pt = (curr_op_pt / max_avail).unsqueeze(-1)
                norm_est_comp = (est_comp_on_mach / max_avail).unsqueeze(-1)
                mach_idle_relative = ((max_avail - machine_avail_time) / max_avail).unsqueeze(-1)

                dyn_mach_feats = torch.cat([norm_mach_avail, norm_curr_op_pt, norm_est_comp, mach_idle_relative],
                                           dim=-1)

            k_mach_concat = torch.cat([base_k_mach, dyn_mach_feats], dim=-1)
            ...
            # 更新排产状态与可用时间 (环境推演，不涉及梯度)
            with torch.no_grad():
                batch_idx = torch.arange(bsz, device=device)
                chosen_pt = op_proc_time[batch_idx, sel_node_idx.squeeze(1), selected_mach]

                chosen_job_ready = torch.gather(job_ready_time, 1, sel_job_unsq).squeeze(1)
                chosen_mach_avail = torch.gather(machine_avail_time, 1, selected_mach.unsqueeze(1)).squeeze(1)
                ...
                one_hot = F.one_hot(selected_job, num_classes=n_j)
            ...
            selected_mach_emb = torch.gather(mach_dense, 1, sel_mach_expanded).squeeze(1)
